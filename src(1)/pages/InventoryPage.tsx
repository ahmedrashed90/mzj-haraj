...
export function InventoryPage() {
  ...
  const suggestedLabel = new Date().getDay() === 5 ? "الجدول الأسبوعي الجديد" : "الجدول الحالي";
  ...
  return <>
    ...
    <section className="next-plan-card">
      <div className="next-plan-icon"><CalendarBlank size={25} weight="duotone" /></div>
      <div className="next-plan-copy">
        <span>{suggestedLabel} · فترة النشر التي سيستخدمها زر تجهيز الجدول</span>
        <strong>{formatPlanRange(suggested.planStart, suggested.planEnd)}</strong>
        <small>{new Date().getDay() === 5 ? "يوم الجمعة: يتم تجهيز الجدول الجديد تلقائيًا من السبت إلى الجمعة حسب حدود الفروع الحالية." : "هذا الجدول يبدأ من بكرة وينتهي يوم الجمعة. يوم الجمعة القادم سيبدأ الجدول الجديد من السبت إلى الجمعة."}</small>
      </div>
      <div className="next-plan-days"><b>{suggestedDays}</b><span>أيام نشر</span></div>
    </section>
    ...
    {showPlan ? <div className="modal-backdrop" onMouseDown={(e) => { if (e.currentTarget === e.target && !saving) setShowPlan(false); }}>
      <div className="modal-card weekly-plan-modal">
        ...
        <div className="period-card locked-period"><CalendarBlank size={23} /><div><span>فترة النشر المعتمدة تلقائيًا</span><strong>{formatPlanRange(planStart, planEnd)}</strong><small>{new Date().getDay() === 5 ? "السبت إلى الجمعة · حسب حدود الفروع الحالية وقت إنشاء الجدول" : "من بكرة إلى الجمعة · هذه الفترة ثابتة لهذا الجدول"}</small></div><span className="locked-period-badge">{getPlanDays(planStart, planEnd).length} أيام</span></div>
