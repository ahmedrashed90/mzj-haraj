    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          <div className="admin-mini"><div className="avatar">A</div><div><strong>Admin</strong><span>{user?.email || ""}</span></div></div>
          <button className="logout-button" onClick={() => void logout()}><SignOut size={18} />تسجيل الخروج</button>
        </div>
      </aside>
      <main className="main-area">
        <header className="topbar">
          <div><strong>{import.meta.env.VITE_APP_NAME || "MZJ Haraj Manager"}</strong><span>جدولة الفروع · تغطية الاستوك · متابعة روابط الإعلانات</span></div>
