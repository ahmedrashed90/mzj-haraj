...
export function DistributionPage() {
  ...
  const totalCapacity = useMemo(() => getPlanTotalCapacity(accounts, planStart, planEnd), [accounts, planStart, planEnd]);
  ...
  const branches = useMemo(() => accounts.map((account) => {
    ...
    const branchCapacity = getAccountPlanCapacity(account, planStart, planEnd);
    ...
    return { account, activeBranchAgents, branchAds, branchCapacity, agentRows };
  }), [accounts, agents, weekAds, planStart, planEnd]);
  ...
  return <>
    <PageTitle
      title="توزيع الفروع"
      subtitle={`متابعة توزيع خطة ${formatPlanRange(planStart, planEnd)}. حد كل فرع يومي، والتكليفات تذهب فقط لمناديب نفس الفرع.`}
    ...
    <section className="stats-grid compact-stats">
      <StatCard label="سعة الفترة" value={totalCapacity} hint="الحد اليومي × أيام الفترة" tone="info" />
      <StatCard label="تكليفات الفترة" value={weekAds.length} hint="حسب حدود الفروع اليومية" tone="good" />
    ...
    {!branches.length ? <section className="panel"><EmptyState title="لا توجد فروع" text="أضف الفروع والمناديب أولًا." /></section> : <div className="branch-distribution-stack">
      {branches.map(({ account, activeBranchAgents, branchAds, branchCapacity, agentRows }) => <section className="panel branch-distribution-panel" key={account.id}>
        <div className="branch-panel-head"><div><span>الفرع</span><h2>{account.name}</h2><p>{activeBranchAgents.length} مندوب نشط</p></div><div className="branch-panel-metrics"><span>الحد اليومي <b>{account.adLimit}</b></span><span>حد الفترة <b>{branchCapacity}</b></span><span>مجدول <b>{branchAds.length}</b></span><span>تم النشر <b>{branchAds.filter(isPublished).length}</b></span></div></div>
        <Progress value={branchAds.length} max={Math.max(branchCapacity, branchAds.length || 1)} />
