...
export function DistributionPage() {
  ...
  return <>
    <PageTitle title="توزيع المناديب" subtitle={`حساب حراج واحد (${publishingSettings.accountName || "غير محدد"}). العدد اليومي يتقسم على الفروع حسب المناديب النشطين، ثم تتوزع حصة كل فرع على مناديبه — ${formatPlanRange(planStart, planEnd)}.`} actions={<div className="week-switcher"><button className="icon-button" onClick={() => goWeek(-1)}><ArrowRight size={19} /></button><label><CalendarBlank size={18} /><input type="date" value={weekStart} onChange={(e) => selectWeek(e.target.value)} /></label><button className="icon-button" onClick={() => goWeek(1)}><ArrowLeft size={19} /></button></div>} />
    <section className="stats-grid compact-stats">
      ...
      <StatCard label="المناديب النشطون" value={activeAgents.length} hint={`داخل ${branchRows.filter((row) => row.reps).length} فروع`} />
    </section>

    {branchRows.length ? <section className="panel"><div className="panel-head"><div><h2>حصة كل فرع من التكليفات</h2><p>الحصة ليست حد حراج مستقل؛ هي نتيجة تقسيم حد الحساب الواحد حسب عدد المناديب النشطين في كل فرع.</p></div></div><div className="branch-summary-grid">{branchRows.map(({ branch, count, reps }) => <div className="branch-summary-card" key={branch.id}><strong>{branch.name}</strong><span>{reps} مندوب نشط</span><b>{count} تكليف في الفترة</b></div>)}</div></section> : null}

    <section className="panel">
      <div className="panel-head"><div><h2>توزيع المناديب داخل الفروع</h2><p>بعد تحديد حصة الفرع، النظام يوازن التكليفات بين مناديب هذا الفرع حسب اليوم ثم إجمالي الفترة ثم التاريخ السابق.</p></div></div>
