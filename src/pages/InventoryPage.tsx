...
export function InventoryPage() {
  ...
  return <>
    ...
    {showPlan ? <div className="modal-backdrop" onMouseDown={(e) => { if (e.currentTarget === e.target && !saving) setShowPlan(false); }}><div className="modal-card weekly-plan-modal">
      <div className="modal-head"><div><h2>مراجعة جدول النشر قبل الاعتماد</h2><p>الجدول يتبع فترات النشر المحددة وترتيب المناديب داخل كل فترة. لا تتكرر السيارة داخل نفس دورة التغطية.</p></div><button className="icon-button" onClick={() => setShowPlan(false)} disabled={saving}>×</button></div>
