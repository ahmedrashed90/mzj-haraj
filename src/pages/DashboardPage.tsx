...
export function DashboardPage() {
  ...
  const fallbackPlan = suggested.weekStart === currentWeek
    ? { planStart: suggested.planStart, planEnd: suggested.planEnd }
    : { planStart: currentWeek, planEnd: getWeekEndKey(currentWeek) };
  const currentPlan = weekAds.length ? getPlanWindowFromAds(weekAds, currentWeek) : fallbackPlan;
  const totalCapacity = getPlanTotalCapacity(accounts, currentPlan.planStart, currentPlan.planEnd);
  ...
  const accountUsage = useMemo(() => accounts.map((account) => ({
    ...
    capacity: getAccountPlanCapacity(account, currentPlan.planStart, currentPlan.planEnd),
  })), [accounts, weekAds, currentPlan.planStart, currentPlan.planEnd]);
  ...
  return <>
    ...
    <section className="stats-grid dashboard-stats">
      <StatCard label="سعة الفترة الحالية" value={totalCapacity} hint="الحد اليومي × أيام الفترة" tone="info" />
      <StatCard label="المجدول في الفترة" value={weekAds.length} hint={`متبقي ${remaining} من السعة`} tone="good" />
      ...
      <StatCard label="سيارات متاح للبيع" value={stockTotalVehicles} hint={`${stock.length} سيارة/فئة خارج الوكالة`} tone="info" />
      ...
      <StatCard label="بدون رابط" value={withoutUrl} hint="من تكليفات الفترة" tone={withoutUrl ? "danger" : "good"} />
    </section>

    <section className="dashboard-columns">
      <div className="panel">
        <div className="panel-head"><div><h2>الفروع وحسابات حراج — الفترة الحالية</h2><p>الحد المسجل لكل فرع يومي، والسعة الظاهرة هي مجموع أيام الفترة.</p></div></div>
        {!accountUsage.length ? <EmptyState title="لا توجد حسابات" text="أضف حسابات حراج أولًا." /> : <div className="account-usage-list">
          {accountUsage.map(({ account, used, capacity }) => <div className={`usage-card ${used > capacity ? "over-limit" : ""}`} key={account.id}>
            <div className="usage-top"><div><strong>{account.name}</strong><span>الحد اليومي: {account.adLimit} · حد الفترة: {capacity}</span></div><b>{Math.max(0, capacity - used)} متبقي</b></div>
            <Progress value={used} max={Math.max(capacity, used || 1)} />
          </div>)}
        </div>}
      </div>
    ...
    <section className="dashboard-columns dashboard-secondary">
      <div className="panel">
        ...
        {!todayAds.length ? <EmptyState title="لا توجد إعلانات اليوم" text="راجع جدول النشر أو أنشئ خطة جديدة." /> : <div className="today-task-list">
      ...
      <div className="panel">
        ...
        {!recentAds.length ? <EmptyState title="لا توجد إعلانات" text="ابدأ من مخزون السيارات وأنشئ أول جدول نشر." /> : <div className="compact-list">
