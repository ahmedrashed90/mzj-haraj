...
export function AdsPage() {
  ...
  }, [ads, status, accountId, agentId, urlFilter, search, accountById, agentById]);

  return <>
    <PageTitle title="الإعلانات" subtitle="كل سيارة مرتبطة بحساب حراج ومندوب، ويمكنك إضافة رابط الإعلان وفتحه مباشرة للمراجعة." />
    ...
    <section className="panel">
      {!rows.length ? <EmptyState title="لا توجد إعلانات" text="اختَر سيارة من صفحة مخزون السيارات وأنشئ تكليفًا." /> : <div className="table-scroll"><table className="ads-table"><thead><tr><th>السيارة</th><th>البيان</th><th>موديل</th><th>الحساب</th><th>المندوب</th><th>الحالة</th><th>الرابط والملاحظات</th><th></th></tr></thead><tbody>
        {rows.map((ad) => <tr key={ad.id}>
          ...
          <td><ConfirmButton className="icon-danger" confirmText="حذف سجل الإعلان من النظام؟" onConfirm={() => removeAd(ad.id)}><Trash size={17} /></ConfirmButton></td>
        </tr>)}
      </tbody></table></div>}
    </section>
  </>;
