...
async function verifyFirebaseAdmin(idToken: string) {
  ...
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${encodeURIComponent(FIREBASE_API_KEY)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ idToken }) });
  if (!response.ok) return false;
  const payload = await response.json() as { users?: Array<{ email?: string; disabled?: boolean }> };
  const user = payload.users?.[0]; return Boolean(user && !user.disabled && clean(user.email).toLowerCase() === ADMIN_EMAIL.toLowerCase());
}
function normalize(raw: RawCar) {
  return {
    ...
    make: clean(raw.make), model: clean(raw.model), trim: clean(raw.trim), year: clean(raw.year), body: clean(raw.body),
    transmission: clean(raw.transmission), drivetrain: clean(raw.drivetrain), engine: clean(raw.engine), fuel: clean(raw.fuel),
    compareKey: clean(raw.compare_key ?? raw.compareKey), permalink: clean(raw.permalink),
    baseSpecs: record(raw.base_specs ?? raw.baseSpecs), interiorSpecs: list(raw.interior_specs ?? raw.interiorSpecs), exteriorSpecs: list(raw.exterior_specs ?? raw.exteriorSpecs), safetySpecs: list(raw.safety_specs ?? raw.safetySpecs),
  };
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const response = await fetch(url, { method: "GET", headers: { accept: "application/json", "x-mzj-bridge-key": secret, "user-agent": "MZJ-Haraj-Manager/1.7" }, cache: "no-store" });
